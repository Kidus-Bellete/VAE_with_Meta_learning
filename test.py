...
def main():
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='./mvtec/pill/vgg_feature/seed_6311/pill_2024-07-30-6906_model.pt')
    ...
    #args.save_dir = './' + args.data_type + '/' + args.obj + '/vgg_feature' + '/results/'
...
def test(model, teacher, test_loader):
    ...
    for (data, label, mask) in tqdm(test_loader):
        ...
        with torch.no_grad():
            ...
            #sum(1, keepdim=True) suggests that the loss is summed across dimensions other than the batch dimension, 
            # resulting in a tensor where each element represents the MSE loss for a corresponding batch element.
            for i in range(len(s_activations)):
                ...
                #F.interpolate is a PyTorch function used for upsampling the mse_loss tensor to match the size
                # of data along the spatial dimensions

        score = score.squeeze().cpu().numpy()
        #print('len score',len(score))
        for i in range(score.shape[0]):
            score[i] = gaussian_filter(score[i], sigma=4)  #Smoothing helps to reduce noise and refine the score values
        scores.extend(score)
...
def plot_fig(args, test_img, recon_imgs, scores, gts, threshold, save_dir):
    ...
    for i in range(num):
        ...
        fig_img.savefig(os.path.join('./results', args.obj + '_{}_png'.format(i)), dpi=100)
        plt.close()
