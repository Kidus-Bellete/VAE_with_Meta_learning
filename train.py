...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='./train_patches')
    ...
    #args.save_dir = './' + args.data_type + '/' + args.obj + '/vgg_feature' + '/model/'
    ...
    teacher = models.vgg19(pretrained=True).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        #reptile_train(args, model, teacher, epoch, train_loader, optimizer, log)
        #maml_train(args, model, teacher, epoch, train_loader, optimizer, log)
...
def m_train(args, model, teacher, epoch, train_loader, optimizer, log):
    model.train()
    teacher.eval()
    losses = AverageMeter()
    MSE_loss = nn.MSELoss(reduction='sum')
    ...
    for (data, _, _) in tqdm(train_loader):
        ...
        inner_optimizer = optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        inner_optimizer.zero_grad()
        data = data.to(device)
        support_set, query_set = data.split([int(0.8 * len(data)), len(data) - int(0.8 * len(data))])
        z, output, mu, log_var = model(support_set)
        s_activations, _ = feature_extractor(z, model.decode, target_layers=['10', '16', '22'])
        t_activations, _ = feature_extractor(support_set, teacher.features, target_layers=['7', '14', '21'])
        mse_loss = MSE_loss(output, support_set)
        #optimizer.zero_grad()
        #mse_loss = MSE_loss(output, data)
        kld_loss = 0.5 * torch.sum(-1 - log_var + torch.exp(log_var) + mu**2)
        for i in range(len(s_activations)):
        ...
        optimizer.zero_grad()
        z, output, mu, log_var = model(query_set)
        s_activations, _ = feature_extractor(z, model.decode, target_layers=['10', '16', '22'])
        t_activations, _ = feature_extractor(query_set, teacher.features, target_layers=['7', '14', '21'])
        mse_loss = MSE_loss(output, query_set)
        kld_loss = 0.5 * torch.sum(-1 - log_var + torch.exp(log_var) + mu**2)
        for i in range(len(s_activations)):
            s_act = model.adapter[i](s_activations[-(i + 1)])
            mse_loss += MSE_loss(s_act, t_activations[i])
...
def reptile_train(args, model, teacher, epoch, train_loader, optimizer, log):
    model.train()
    teacher.eval()
    losses = AverageMeter()
    MSE_loss = nn.MSELoss(reduction='sum')
    inner_lr = args.lr  # Inner learning rate
    ...
    for (data, _, _) in tqdm(train_loader):
        ...
        for _ in range(5):  # Number of inner loop steps
            inner_optimizer = optim.Adam(model.parameters(), lr=inner_lr, weight_decay=args.weight_decay)
            inner_optimizer.zero_grad()
        ...
        for name, param in model.named_parameters():
            param.data = initial_params[name] + (param.data - initial_params[name]) * 0.1  # 0.1 is the meta learning rate
...
def maml_train(args, model, teacher, epoch, train_loader, optimizer, log):
    model.train()
    teacher.eval()
    losses = AverageMeter()
    MSE_loss = nn.MSELoss(reduction='sum')
    ...
    for (data, _, _) in tqdm(train_loader):
        ...
        # MAML update
        for name, param in model.named_parameters():
            param.data = initial_params[name] + (param.data - adapted_params[name])
